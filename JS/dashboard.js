...
document.addEventListener('DOMContentLoaded', function () {
  ...
  window._budgetDonut = new Chart(ctxBudget, {
    ...
    data: {
      ...
      datasets: [{ data: [62, 38], backgroundColor: ['#0D1F3C', '#C4A84F'], borderWidth: 0, hoverOffset: 6 }]
    },
  ...
  window._tenantDonut = new Chart(ctxTenants, {
    ...
    data: {
      ...
      datasets: [{ data: [71, 29], backgroundColor: ['#0D1F3C', '#C4A84F'], borderWidth: 0, hoverOffset: 6 }]
    },
    options: sharedOptions
  });
...
window.updateDashboardDonuts = function (totalBudget, totalExpenses, tenantPaid, tenantDue) {
  ...
  const tenantTotal = tenantPaid + tenantDue;
  const tenantPct   = tenantTotal > 0 ? Math.round((tenantPaid / tenantTotal) * 100) : 0;

  if (window._tenantDonut) {
    window._tenantDonut.data.datasets[0].data = [tenantPaid, tenantDue];
    ...
    // Update tenant legend with amounts
    const elPaid = document.getElementById('tenantLegendPaid');
    const elDue  = document.getElementById('tenantLegendDue');
    if (elPaid) elPaid.textContent = '$' + tenantPaid.toLocaleString('en-US', { minimumFractionDigits: 0 });
    if (elDue)  elDue.textContent  = '$' + tenantDue.toLocaleString('en-US',  { minimumFractionDigits: 0 });
  }
};
...
(function () {
  ...
  const COLORS = [
    '#0D1F3C', '#0D1F3C', '#1A3A6B', '#7AAEDD',
    '#C4A84F', '#6B7FA3', '#3D5A80', '#8FB4D4',
...
(function () {
  ...
  function renderBarChart(labels, budget, expenses) {
    const expColors = expenses.map(function (e, i) {
      return e > budget[i] ? '#8B7536' : '#1A3A6B';
    });
  ...
  window.loadSupplierCharts = function (month, year) {
    ...
    fetch(url)
      ...
      .then(function (res) {
        ...
        if (supplierSel.options.length <= 1 && res.suppliers.length) {
          supplierSel.innerHTML = '';
          res.suppliers.forEach(function (s) {
            const opt = document.createElement('option');
            opt.value = s.Supplier_ID;
            opt.textContent = s.Name;
            if (s.Supplier_ID == '1361') opt.selected = true;
            supplierSel.appendChild(opt);
          });
          // Reload with default supplier
          if (!sid) {
            window.loadSupplierCharts(
              document.getElementById('dashMonthSelect') ? document.getElementById('dashMonthSelect').value : '',
              document.getElementById('dashYearSelect')  ? document.getElementById('dashYearSelect').value  : ''
            );
            return;
          }
        }
  ...
  function renderTotalChart(paid, due) {
    ...
    if (totalChart) {
      ...
      totalChart = new Chart(ctx, {
        ...
        data: {
          labels: ['Paid', 'Due'],
          datasets: [{ data: [paid, due], backgroundColor: ['#0D1F3C', '#C4A84F'], borderWidth: 0, hoverOffset: 6 }]
        },
        options: DONUT_OPTS
      });
    }
