...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function updateSummary(data) {
    ...
    if (countPaid)   countPaid.textContent   = data.filter(t => parseFloat(t.due_amount) === 0).length;
    if (countUnpaid) countUnpaid.textContent = data.filter(t => parseFloat(t.due_amount) !== 0).length;
  }
